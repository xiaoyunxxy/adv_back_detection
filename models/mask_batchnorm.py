...
class MaskNoisyBatchNorm2d(nn.BatchNorm2d):
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=True,
                 track_running_stats=True):
        super(MaskNoisyBatchNorm2d, self).__init__(
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        if self.is_perturbed:
            coeff_weight = self.neuron_mask*self.weight + (1-self.neuron_mask)*self.neuron_noise
            coeff_bias = 1.0 + self.neuron_noise_bias
        else:
            coeff_weight = self.neuron_mask*self.weight
            coeff_bias = 1.0
        return F.batch_norm(
            input,
            ...
            coeff_weight, self.bias * coeff_bias,
